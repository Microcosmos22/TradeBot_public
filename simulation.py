...
def load_simdata(sim_N):
    ...
    features_full = np.load("../features_sim.npy")
    features_large_full = np.load("../features_sim_large.npy")
    dates = np.load("../dates_sim.npy")
    
    #target = np.load("../target_sim.npy")
    #features = np.load("../features_sim.npy")
    
    return target_full, features_full, features_large_full, dates
...
def load_machines_scalers(onlyfirst = None):
    ...
    machinepaths = os.listdir("../machines/tracking_machines_mixed")
    scalerpaths = os.listdir("../scalers")
    
    machines = [mpath for mpath in machinepaths if os.path.isfile(os.path.join("../machines/tracking_machines_mixed", mpath))]
    ...
    for i in range(len(machines)):
        if os.path.isfile(os.path.join("../machines/tracking_machines_mixed", machines[i])):
            ...
            #print("looking for scaler")
            
            print(os.path.join("..\scalers", scalername[:-2]+"pkl"))
            #print(os.path.isfile(os.path.normpath(os.path.join("..\scalers", scalername[:-2]+"pkl"))))
            ...
            if os.path.isfile(os.path.normpath(os.path.join("..\scalers", scalername[:-2]+"pkl"))): # scaler exists
                ...
                model = load_model(os.path.join("../machines/tracking_machines_mixed", machines[i]))
                with open(os.path.join("..\scalers", scalername[:-2]+"pkl"), 'rb') as f:
                    scaleri = pickle.load(f)
                    
                print("Scaler expects {} features".format(scaleri.n_features_in_))
...
def simulate_rendite(sim_N = 3600, verboseeach = 250, onlyfirstmachines=None, equil_const = 2, subsets = 200):
    ...
    target_full, features_full, features_large_full, dates = load_simdata(sim_N)
    ...
    for i in range(len(targetn)): # Loop through the hourly tapes
        ...
        for k in range(len(modeln)):
            ...
            elif scalern[k].n_features_in_ == 5:
    ...
    for k in range(len(modeln)):
        print(" Haben einen Gewinn von {:.2f} $, korrigiert {:.2f} $, Treffer {:.2f} %, Avg bet {:.2f} $  ".format((portfolio[k]-20000),(portfolio[k]-20000)-btc_gewinn, np.mean(np.asarray(correct_direction[k]))*100, np.mean(bets[k])))
        
        plt.plot(port_track[k])
        plt.savefig(machinestrn[k][:-4]+".png")
